...
|---|---|---|---|
| T-26b | FeasibilityModel per-knob feature weights OR per-FailureKind sub-classifiers | Campaign 02 confirmed class-collapse on `kv_cache_dtype` alone is insufficient — `_config_distance` averages distance across 12 knobs, so the per-knob class signal is diluted by 11 unrelated knobs varying. fp8 proposals after first fp8 failure: 7 (target ≤ 1). L1 surrogate kept-rate stuck at 8% same as Campaign 01 baseline. Q2 needs this fix to clear the 30% threshold. | `policy/feasibility.py:_config_distance` |
